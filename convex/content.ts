...
export const listTrending = query({
  ...
  handler: async (ctx, { limit = 6 }) => {
    ...
    return Promise.all(top.map((doc) => attachFeedAndNiches(ctx, doc)));
  },
});
...
export const getBySlug = query({
  ...
  handler: async (ctx, { slug }) => {
    const doc = await ctx.db
      ...
      .first();
    if (!doc) return null;
    return attachFeedAndNiches(ctx, doc);
  },
});
...
async function attachFeedAndNiches(ctx: any, doc: any, overrideFeedSlug?: string, overrideFeedName?: string) {
  ...
  return {
    ...
    body: doc.body,
    summary: doc.summary,
    excerpt: doc.summary ?? (doc.body ? doc.body.slice(0, 200) + "…" : null),
    status: doc.status,
    authorId: doc.authorId ?? null,
    publishedAt: doc.publishedAt ?? null,
    published_at: doc.publishedAt != null ? new Date(doc.publishedAt).toISOString() : null,
    created_at: null,
    updated_at: null,
