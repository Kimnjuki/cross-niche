...
export default function BlogSeries() {
  ...
  const handleFixExisting = async () => {
    ...
    try {
      ...
      if (result.success) {
        const r = result as { updated?: number; linkedToFeeds?: number; linkedToNiches?: number; visibleCount?: number };
        toast.success('Content fixed successfully', {
          description: `Updated ${r.updated ?? 0} articles, linked ${r.linkedToFeeds ?? 0} to feeds, ${r.linkedToNiches ?? 0} to niches. ${r.visibleCount ?? 0} articles now visible.`
        });
      } else {
        toast.error('Fix failed', {
