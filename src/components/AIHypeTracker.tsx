...
export function AIHypeTracker() {
  const analysis = useQuery(api.aiUpdates.list);
  
  if (!analysis) return null;
  
  const hypeCount = analysis.filter((u) => u.isHype).length;
  const substantiveCount = analysis.length - hypeCount;
  const hypePercentage = analysis.length > 0 ? (hypeCount / analysis.length) * 100 : 0;
  ...
  return (
    <div className="bg-gradient-to-r from-purple-500 to-pink-500 p-6 rounded-lg text-white">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          <div className="text-3xl font-bold">{substantiveCount}</div>
          <div className="text-sm opacity-90">Substantive</div>
        </div>
        <div>
          <div className="text-3xl font-bold">{hypeCount}</div>
      ...
      <div className="mt-4">
        ...
        <p className="text-sm mt-2">
          {hypePercentage.toFixed(0)}% of AI news is pure hype
        </p>
      </div>
    </div>
  );
