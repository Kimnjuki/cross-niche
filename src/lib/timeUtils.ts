...
/**
 * Compact relative time for LiveWire/NewsFeed (e.g., "5m ago", "2h ago")
 */
export function formatCompactRelativeTime(dateString: string | number | null | undefined): string {
  ...
  const diffDay = Math.floor(diffHr / 24);  if (diffSec < 60) return 'Just now';
