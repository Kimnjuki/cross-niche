...
export async function insertArticles() {
  ...
  try {
    // First, get or create necessary feeds and niches
    ...
    const requiredFeeds = [
      { id: 1, slug: 'innovate', name: 'Innovate', color_code: '#3b82f6', display_order: 1 },
      { id: 2, slug: 'secured', name: 'Secured', color_code: '#ef4444', display_order: 2 },
      { id: 3, slug: 'play', name: 'Play', color_code: '#10b981', display_order: 3 },
    ];

    for (const feed of requiredFeeds) {
      const existing = feeds?.find(f => f.slug === feed.slug);
      if (existing) {
        feedMap[feed.slug] = existing.id;
      } else {
    ...
    const requiredNiches = [
      { id: 1, name: 'tech', color_code: '#3b82f6' },
      { id: 2, name: 'security', color_code: '#ef4444' },
      { id: 3, name: 'gaming', color_code: '#10b981' },
    ];

    for (const niche of requiredNiches) {
      const existing = niches?.find(n => n.name === niche.name);
      if (existing) {
        nicheMap[niche.name] = existing.id;
      } else {
    ...
    // Map niche to feed slug
    const nicheToFeed: Record<string, string> = {
      tech: 'innovate',
      security: 'secured',
      gaming: 'play',
    };
    ...
    for (const articleData of articles) {
      ...
      const htmlContent = formatContentAsHTML(articleData.content);
      ...
      if (existing) {
        ...
        results.push({ title: articleData.title, status: 'skipped', id: existing.id });
        continue;
      }

      // Insert article
      const contentInsert: TablesInsert<'content'> = {
        ...
        featured_image_url: articleData.featuredImageUrl || '/placeholder.svg',
        seo_meta_title: articleData.title,
        seo_meta_description: articleData.excerpt,
      ...
      if (contentError || !content) {
        console.error(`Error inserting article "${articleData.title}":`, contentError);
        results.push({ title: articleData.title, status: 'error', error: contentError?.message });
        continue;
      }

      // Link to feed
      const feedSlug = nicheToFeed[articleData.niche];
      const feedId = feedMap[feedSlug];
      if (feedId) {
        await supabase.from('content_feeds').insert({
      ...
      // Link to niche
      const nicheId = nicheMap[articleData.niche];
      if (nicheId) {
        await supabase.from('content_niches').insert({
          content_id: content.id,
          niche_id: nicheId,
        });
      }
      ...
      // Insert tags
      const allTags = [...articleData.tags, ...articleData.seoKeywords];
      ...
      results.push({ title: articleData.title, status: 'success', id: content.id });
    }
